...
static std::vector<AccessEvent>
build(std::vector<std::unique_ptr<ApNode>> nodes)
{
...
TEST(EventBuilder, loop_stack_records_active_iterations)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    {
      int idx = i * 3 + j;
      ASSERT_EQ(events[idx].loop_stack.size(), 2u);
      EXPECT_EQ(events[idx].loop_stack[0].var, "i");
      EXPECT_EQ(events[idx].loop_stack[0].iter, i);
      EXPECT_EQ(events[idx].loop_stack[1].var, "j");
      EXPECT_EQ(events[idx].loop_stack[1].iter, j);
    }
  }
}
