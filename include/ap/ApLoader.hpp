...
public:
     ...
     */
    std::vector<std::unique_ptr<ApNode>> load_json_string(const std::string& json) const;

    /**
     * @brief AP JSON 파일을 파싱해 ApNode 목록을 반환한다.
     * @throws std::runtime_error 파일을 열 수 없거나 shape 정보가 없을 때
     */
    std::vector<std::unique_ptr<ApNode>> load_json_file(const std::string& path) const;
...
private:
    ...
    std::vector<std::unique_ptr<ApNode>> parse_toplevel(const std::string& json) const;
    std::unique_ptr<ApNode> parse_node(const void* j, const ShapeMap& shapes) const;
};
