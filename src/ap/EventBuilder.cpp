...
std::vector<AccessEvent> EventBuilder::build(
    std::vector<std::unique_ptr<ApNode>> nodes)
{
    ...
    for (const auto& n : nodes)
        visit(*n, out, loop_stack, "", seq);
    return out;
}

// ── private ───────────────────────────────────────────────────

void EventBuilder::visit(const ApNode&            node,
                          std::vector<AccessEvent>& out,
                          std::vector<LoopFrame>&   loop_stack,
                          ...
                          uint64_t&                 seq)
{
    ...
    case ApNodeKind::Call: {
        ...
        if (it == func_map_.end())
            break;
        const std::string path = region_path.empty()
            ? c.callee
            : region_path + "/" + c.callee;
